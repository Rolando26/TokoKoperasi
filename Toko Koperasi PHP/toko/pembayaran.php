<?php
  ...
  if(!empty($_GET)){
	...
	$q = mysql_query("select*from pesanan where user_id='$user->id' And status='belum lunas'");
	$j = mysql_num_rows($q);
?>
...
<?php
		if($_GET['act'] == 'bayar' && $_GET['id']){
			if(!empty($_POST)){
				...
				extract($_POST);
				$q = mysql_query("insert into pembayaran Values(NULL,'$_GET[id]','$user->id','$gambar','$bayar','pending','$keterangan',NOW())");
				if($q){
