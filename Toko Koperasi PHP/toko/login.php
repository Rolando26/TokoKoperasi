<?php 
	...
	if(!empty($_POST)){
		...
		$q = mysql_query("SELECT * FROM `user` WHERE email='$email' AND password='$password' AND status='user'");
		$a = mysql_query("SELECT * FROM `user` WHERE email='$email' AND password='$password' AND status='admin'");
		if(mysql_num_rows($q) > 0){
			$_SESSION['iam_user'] = $email;
			redir("index.php");
...
?> 
		...
		<div class="col-md-9">
			...
			<div class="alert alert-success">Email dan password yang anda masukan tidak terdaftar!!	</div>
				<?php }else{ ?>
